...
- [ ] 完整 workflow 覆盖成功、局部未解决后继续下游、模块级失败回退和主动停止。
- [ ] 独立后处理覆盖资产自动发现、资产缺失备用输入、仅报告和自动修复。
- [ ] 覆盖单语、双语、两侧独立显示策略、自动换行和一对多拆分。
- [ ] 覆盖首次提交、4 次业务重试、网络级重试、重复候选和区域级回退计数。
- [ ] 覆盖过程目录隔离、manifest 验证、成功后导出及所有失败状态不导出。
- [ ] 覆盖旧配置残留不迁移且不影响新默认配置。
- [ ] 非集成 pytest、Ruff 和 Pyright 通过，测试只依赖安全的临时资产和 fake 依赖。
