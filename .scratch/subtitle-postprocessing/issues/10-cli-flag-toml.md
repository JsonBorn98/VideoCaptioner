...
- [ ] flag→postprocess.* 映射测试（parser Namespace 先例），postprocess 与 process 两命令都可用
- [ ] TOML 6 键→replace(resolved) 覆盖测试（_CONFIG_OVERRIDE_FIELDS 消费路径），限长 4 值经 TOML 可调（无 flag）
- [ ] 非法 flag 值（argparse choices）与非法 TOML 组合（目标>绝对）报 USAGE_ERROR 不静默
- [ ] 默认态（无 flag 无 TOML）：键不出现在 overrides，方案 JSON 值生效
