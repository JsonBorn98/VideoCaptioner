...
class ASRData:
    ...
    def to_ass(
        ...
        layout_marker = layout.name if isinstance(layout, SubtitleLayoutEnum) else str(layout)
        ass_content = (
            "[Script Info]\n"
    ...
    def from_ass(
        ...
        if layout is None:
            ...
            if layout_match:
                layout_token = layout_match.group(1)
                try:
                    layout = SubtitleLayoutEnum[layout_token]
                except KeyError:
                    try:
                        layout = SubtitleLayoutEnum(layout_token)
                    except ValueError:
        ...
        def assign_bilingual_text(segment: ASRDataSeg, style: str, text: str) -> None:
            if layout == SubtitleLayoutEnum.TRANSLATE_ON_TOP:
                # VideoCaptioner writes translate-on-top ASS as:
                # Secondary = original (lower), Default = translation (upper).
                if style == "Default":
                    segment.translated_text = text
                else:
                    segment.text = text
            else:
                # Backward-compatible default and original-on-top output:
                # Default = original, Secondary = translation.
                if style == "Default":
                    segment.text = text
                else:
                    segment.translated_text = text
                ...
                if match:
                    ...
                    if has_translation:
                        ...
                        if time_key in temp_segments:
                            assign_bilingual_text(temp_segments[time_key], style, text)
                            ...
                            assign_bilingual_text(segment, style, text)
