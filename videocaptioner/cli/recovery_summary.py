...
无人值守脚本默认继续：命令函数把「无 ``--fresh``」转成打印恢复摘要并返回
「继续」的恢复决定回调、「有 ``--fresh``」转成恒返「从头开始」的回调，
分别传给翻译与后处理两个模块入口。模块入口只在发现匹配检查点时调用
回调，无检查点时两条路径都不打印（spec「入口适配 · CLI」）。
"""
...
TRANSLATION_COMPLETED_LABELS: Mapping[str, tuple[str, Optional[str]]] = {
    "analysis": ("全文分析", None),
    "glossary": ("术语阶段", None),
...
POSTPROCESS_COMPLETED_LABELS: Mapping[str, tuple[str, Optional[str]]] = {
    "phase": ("阶段末检查点", None),
    "rounds": ("修复轮次", "轮"),
}


def _render_completed(summary: RecoverySummary, completed_labels: Mapping[str, tuple[str, Optional[str]]]) -> str:
...
def recovery_decision_callback(
    fresh: bool,
    *,
    completed_labels: Mapping[str, tuple[str, Optional[str]]],
) -> Callable[[RecoverySummary], RecoveryDecision]:
    """把 ``--fresh`` 旗标转为传给模块入口的恢复决定回调。
