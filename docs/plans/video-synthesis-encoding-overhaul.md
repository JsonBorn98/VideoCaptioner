...
> **决策修订（本次）**：改为**只读展示**，不做双向反向编辑。反向编辑（改命令回填控件）列为**后续优化方向**。

- 结构化 `EncodeSettings` 为唯一事实源；预览由 `build_ffmpeg_command` 从当前控件（含自定义参数）**实时只读生成**，供用户核对与复制。
- 单独提供**自定义参数**输入框（`cfg.extra_args` → `EncodeSettings.extra_args`）追加在命令末段——这是用户手动介入编码参数的入口；预览会反映它。
- 字幕滤镜 `-vf ass=…`、`scale` 与输入/输出/临时路径均由系统托管；预览中以占位/实际路径展示。
- 软字幕（流复制）单独展示其内嵌命令近似。
- **后续（双向）**：白名单最佳努力反解析（识别常见标志回填控件、未识别归入自定义参数、往返幂等）——§16.4 的相关约束仅适用于该后续项。
