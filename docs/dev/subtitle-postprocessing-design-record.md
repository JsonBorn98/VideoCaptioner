...
最后更新：2026-09-16。状态：原职责与限长设计轮次、性能治理轮次均已收口；2026-09-16「模块内恢复检查点」grilling 已收口，见文末。下文原轮次的实现差距和验证状态是历史记录，不代表当前代码状态；新轮次见文末。
...
- 勘误（2026-09-15）：本条与 D07 的「上下文快照 / 全文简报」源自当时未获用户单独确认的复用建议，实施 spec 误将其写成过程文件清单；同日裁决从 `UPSTREAM_ASSET_KINDS` 移除 `context`（无 schema、无消费者、无生产者，只留下永久缺失警告）。需要简报复用时重新加回 kind。
...
- P01（本轮第 1 轮回答）：只解决运行卡顿与性能、进度、取消治理；本轮不要求新增停止或重启后的断点恢复，允许重新处理。（2026-09-16 修订：范围限定到期，模块内恢复检查点见文末 R01–R07 与 ADR-0022。）
...
- P05（2026-09-09 本会话停止语义确认）：点击停止后尽快结束本地后处理任务，立即停止排队和重试，尽力中断在途请求，不等待模型自然返回；迟到响应不得写回字幕或触发下游。无法保证服务端终止已收到的请求或停止计费；不交付未完整验收的部分后处理成果，不新增断点恢复。（2026-09-16 修订：「不新增断点恢复」由 R06 替代，停止后保留已写入的恢复检查点；不交付部分成果、阻断下游、迟到结果不得写回等其余语义不变，见 ADR-0022。）
...
## 2026-09-16：模块内恢复检查点（grilling 已收口）

来源：`/matt-flow:grill-with-docs`，三轮共 10 项用户裁决。术语已写入 `CONTEXT.md`（恢复检查点、检查点身份、配置漂移、恢复提示、从头开始、模块内恢复、后处理恢复边界）与 `CONTEXT-TRANSLATION.md`（翻译恢复边界、翻译简报文件、译文检查点、审计检查点）；难逆决策见 [ADR-0022](../adr/0022-ticket.md)。

### 本轮已确认

- R01 范围：模块内恢复。增强型 LLM 上下文翻译与后处理各自内部续跑；workflow 阶段级不自动续跑（ADR-0004 末段该部分保留）；单模型 LLM 翻译与非 LLM 翻译不纳入，继续依赖既有 7 天块级结果缓存。
- R02 触发：自动发现 + 明确提示。独立任务与单个编排任务在模块开始时弹恢复提示（页内阻塞步，同人工术语确认）；批量任务与 CLI 无人值守默认继续，批量行内标注「已从恢复检查点继续」，CLI 打印摘要并提供 `--fresh`。
- R03 翻译恢复边界四级：全文分析完成（翻译简报文件，重新加回 `context` 资产种类并定义 schema）、术语确认完成（权威术语表，已有）、每翻译批（译文检查点，已有写路径，补读回）、每审计批（审计检查点，新增）。全文分析窗口与单个术语不单独落盘。
- R04 后处理恢复边界两类：阶段末（确定性阶段、压缩重译、语义修复完成后）与轮末（修复轮次归并验收后：工作字幕、各问题身份尝试计数、已关闭区域、回退记录、下一轮反馈）。轮内候选不落盘，轮内中断靠网关响应缓存回放，缓存 TTL 由 1 小时延长为 7 天。
- R05 有效性宽松：检查点身份 = 翻译源字幕指纹（字幕编号 + 源文本，不含时间轴）或后处理输入指纹 + 语言对 + 翻译方式。提示词、模型配置方案、后处理配置方案变化记为配置漂移，只在恢复提示列出并在审计报告 / QA 报告标注，不使检查点失效；用户可选从头开始。
- R06 中断类型：失败、主动停止、崩溃都保留检查点。停止仍不交付部分成果、仍阻断下游、迟到结果仍不得写回；崩溃残留靠 manifest 与可读性验证，验不过不提示。
- R07 术语：规范术语「恢复检查点」；阶段 SRT（阶段自动保存）与网关响应缓存都不是恢复检查点。

### 本轮按默认处理（已向用户列出，未反对）

- 显式导入的术语表覆盖检查点中的术语表（显式优先于自动发现，与显式备用资产口径一致）。
- 翻译 `.in-progress` 暂存目录改按源字幕指纹分桶并带恢复 manifest；模块成功发布后删除暂存；「从头开始」时旧检查点随新运行进度覆盖，不预先删除。
- 后处理检查点放在任务过程目录，模块成功后清理；恢复检查点不进入过程资产导出。
- 恢复提示内容：已完成阶段 / 批次数、检查点时间、配置漂移清单；GUI 两个按钮「从检查点继续」「从头开始」。
- 恢复时批规划只覆盖缺失字幕段 / 未审计批；人工术语确认已在术语表阶段完成的不再询问，人工审计确认在全部审计批到齐后进入。
- 检查点损坏或不可读：提示后从头开始，不静默复用、不静默丢弃。
- 恢复后的翻译执行快照与 manifest 记录恢复来源与配置漂移，供审计报告 / QA 报告标注。

### 事实依据（2026-09-16 只读核查）

- 翻译上下文简报当前不落盘；`context` 资产种类 2026-09-15 因无 schema / 生产者 / 消费者移除（见 D21 勘误），本轮决定补生产者后重新加回。
- 译文检查点 `translation-checkpoint.json` 每批原子写（`core/translate/enhanced/runner.py:151-166`），但全仓库无读者；失败信息只把路径告诉用户。
- 后处理 `execute_viewing_repair` 零磁盘 I/O；取消与模块失败丢弃全部内存状态；`postprocess-state.json` 与 QA 报告只在模块成功后写。
- 网关响应缓存键只含请求成形字段、排除 metadata / task_id，`EXPIRE_SECONDS = 3600`（`core/llm/response_cache.py`）；除连接测试外所有 LLM 调用默认启用，因而轮内 / 阶段内重发相同请求可命中。（2026-09-16 已实现：ticket 01 将保留期改为 7 天，见 R04 与 ADR-0022。）
- 单模型 LLM 翻译块缓存 7 天，键含模型 / 方案 / 提示词 / 语言（`core/translate/llm_translator.py:263-280`），是 R01 不纳入的依据。
